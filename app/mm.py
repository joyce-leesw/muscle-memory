...
def main():
    parser = argparse.ArgumentParser(
        description="Add a workout to the database or get workouts for a date"
    )
    ...
    if args.all:
        ...
        try:
            selected_date = datetime.strptime(args.date, "%Y-%m-%d").replace(
                tzinfo=timezone.utc
            )
...
def add_workout(name: str, reps: int, weight: int, sets=int, date=str):
    ...
    try:
        ...
        db.refresh(workout)
        print(
            f"Added workout: {workout.name} - {workout.reps} reps @ {workout.weight}kg x {workout.sets} sets"
        )
    finally:
        db.close()
    ...
    try:
        ...
        if workouts:
            ...
            for workout in workouts:
                print(
                    f"{workout.id}: {workout.name} - {workout.reps} reps @ {workout.weight}kg x {workout.sets} sets on {workout.date.strftime("%Y-%m-%d")}"
                )
        else:
...
def get_workouts_for_date(selected_date: datetime):
    ...
    try:
        start_of_day = datetime.combine(
            selected_date.date(), datetime.min.time()
        ...
        workouts = (
            ...
            .filter(
                models.Workout.date >= start_of_day, models.Workout.date < end_of_day
            )
            .all()
        )
        ...
        if workouts:
            ...
            for workout in workouts:
                print(
                    f"{workout.name} - {workout.reps} reps @ {workout.weight}kg x {workout.sets} sets"
                )
        else:
